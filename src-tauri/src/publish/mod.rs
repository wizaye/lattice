...
/// Build a Markdown landing page for vaults whose root has no
/// `index.md`.  Quartz won't emit `public/index.html` without one —
/// visiting `/` then returns 404 even when every subpage rendered
/// fine.  Most Obsidian-style vaults nest notes under topic folders
/// and leave the root empty, so we hit this on the typical user's
/// first preview.
///
/// The synthesised page lists the top-level folders Quartz will
/// expose plus a count of total notes.  No I/O outside `content_dir`
/// and no allocations beyond what `format!` does — safe to call from
/// the build hot path.
fn synthesize_landing_page(content_dir: &Path, total_notes: u32) -> String {
    let mut top_folders: Vec<String> = Vec::new();
    if let Ok(rd) = std::fs::read_dir(content_dir) {
        for entry in rd.flatten() {
            if entry.file_type().map(|t| t.is_dir()).unwrap_or(false) {
                if let Some(name) = entry.file_name().to_str() {
                    // Skip hidden dirs (`.git`, `.obsidian`, etc) — they
                    // were filtered before the copy loop but defence in
                    // depth is cheap here.
                    if !name.starts_with('.') {
                        top_folders.push(name.to_string());
                    }
                }
            }
        }
    }
    top_folders.sort_by(|a, b| a.to_lowercase().cmp(&b.to_lowercase()));

    let mut body = String::from("---\ntitle: Welcome\n---\n\n");
    if total_notes == 0 {
        body.push_str(
            "Your vault is empty — add some `.md` files and rebuild.\n",
        );
        return body;
    }

    body.push_str(&format!(
        "Welcome to your Lattice-published vault. There are **{}** notes \
         in this preview.\n\n",
        total_notes
    ));
    body.push_str("Use the explorer on the left (or `Ctrl/Cmd+K` search) ");
    body.push_str("to navigate.\n\n");

    if !top_folders.is_empty() {
        body.push_str("## Folders\n\n");
        for folder in &top_folders {
            // Quartz turns folder names with spaces into URL-safe
            // slugs, but it also accepts the raw folder name as a
            // wikilink target — let it do the resolution.
            body.push_str(&format!("- [[{folder}/|{folder}]]\n"));
        }
        body.push('\n');
    }

    body.push_str(
        "> _This landing page was auto-generated by Lattice because \
         your vault has no `index.md` at its root. Create one to \
         replace this page._\n",
    );
    body
}
...
pub async fn publish_build(vault: String) -> Result<String, String> {
    ...
    tokio::task::spawn_blocking(move || -> Result<String, String> {
        ...
        // Ensure `content/index.md` exists.  Quartz only emits
        // `public/index.html` when there's a homepage source — without
        // it, visiting the preview URL `/` returns 404 even though
        // every subpage rendered.  This bit users whose vaults keep
        // all notes in subfolders (the common Obsidian layout).
        //
        // We synthesise a vault-aware landing page that lists the
        // top-level folders + a few recent notes so the preview is
        // immediately useful.  Users who DO have an `index.md` in
        // their vault root keep theirs untouched (the copy loop above
        // ran first; this only writes when the file is still absent).
        let index_path = content_dir.join("index.md");
        if !index_path.exists() {
            let landing = synthesize_landing_page(&content_dir, copied);
            std::fs::write(&index_path, landing)
