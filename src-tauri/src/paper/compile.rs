...
pub fn compile(paper_abs: &Path) -> Result<PathBuf, String> {
    ...
    // 2. Use the EnginePipeline (Strategy pattern) to compile.
    // OCP: adding a new engine only requires implementing CompileEngine
    // and adding it to EnginePipeline::default() — no changes here.
    let pipeline = super::engine::EnginePipeline::default();
    let engine_name = pipeline.compile(&build_dir, "main.tex")?;
...
pub(super) fn assemble_body(paper_abs: &Path) -> Result<String, String> {
    ...
    if sections_dir.is_dir() {
        let mut paths: Vec<PathBuf> = std::fs::read_dir(&sections_dir)
            ...
            .collect();
        // Bug 31 fix: plain `paths.sort()` is lexicographic, which orders
        // `10-conclusion.md` BEFORE `2-methods.md` (because '1' < '2').
        // Use a numeric-aware comparator so the leading decimal prefix
        // determines the sort order even without zero-padding.
        paths.sort_by(|a, b| {
...
/// Natural (numeric-aware) comparison for section filenames.
///
/// Splits each string into alternating non-numeric and numeric segments
/// and compares them so that `10-conclusion` sorts AFTER `2-methods`
/// (because 10 > 2 numerically), not BEFORE it (which is what the
/// default lexicographic `str::cmp` would produce because `'1' < '2'`).
///
/// This is intentionally self-contained (no external crate) because it
/// only needs to handle simple `N-name` prefixes, not arbitrary Unicode.
    ...
    loop {
        match (ai.peek(), bi.peek()) {
            ...
            (Some(ac), Some(bc)) if ac.is_ascii_digit() && bc.is_ascii_digit() => {
                // Collect the full decimal run from each side.
                let num_a: u64 = {
...
mod tests {
    ...
    #[test]
    fn natural_sort_orders_correctly() {
        let mut names = vec![
            "10-conclusion",
            "2-methods",
            "1-introduction",
        ...
        assert_eq!(names, ["1-introduction", "2-methods", "3-results", "10-conclusion"]);
    }
    ...
    fn template_wraps_body_with_documentclass_and_maketitle() {
        ...
        assert!(tex.contains("\\documentclass[10pt]{article}"));
